...
### Repo-Hygiene — was den Build bricht

Im Repository lagen zwischenzeitlich zwei Implementierungen nebeneinander. Benutzt wird
ausschließlich `src/lib/**`, `src/features/{scale,export,parameters,stage}/**`,
`src/store/editor.ts`, `src/components/ui/{button,field,overlay,label}.tsx`.

| Fremder Baum | Wirkung auf den Build |
| --- | --- |
| `src/core/**` | Type-Check scheitert: `Cannot find module 'polygon-clipping'` |
| `src/features/scale-editor/**` | zweite Anwendung, importiert `src/core` |
| `src/app/api/**` | `export const dynamic = "force-dynamic"` ist mit `output: "export"` unvereinbar — **muss gelöscht werden**, ein tsconfig-Ausschluss hilft hier nicht, weil Next Routen über das Dateisystem findet |
| `src/db/**`, `drizzle.config.json` | Datenbankcode, der zur Laufzeit `DATABASE_URL` verlangt |

Zwei Werkzeuge halten das sauber:

- `bash scripts/cleanup-stale.sh` — löscht die fremden Bäume (idempotent).
- `node scripts/check-deploy.mjs` — Selbstprüfung ohne Build: Server-only-Code, tote
  Bäume, fehlende Paketdeklarationen, nötige Dateien, Vollständigkeit. Exit-Code 0 =
  deploybar.

Als Netz ist `src/core` und `src/features/scale-editor` in `tsconfig.json` ausgeschlossen,
sodass ein vergessener Baum den Type-Check nicht mehr bricht. Route-Handler lassen sich
so **nicht** ausschließen — dafür gibt es die Selbstprüfung.
